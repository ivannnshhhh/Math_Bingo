...
shaker.addEventListener("click", () => {
    ...
    const allDone = letters.every(
        (letter) =>
            drawnEquationsPerRound[currentRound][letter].length >=
            equations[currentRound][letter].length,
    );
    if (allDone) {
        return;
    }
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            const availableLetters = letters.filter(
                (l) =>
                    drawnEquationsPerRound[currentRound][l].length <
                    equations[currentRound][l].length,
            );
            ...
            const availableEqs = eqList.filter(
                (e) => !drawnEquationsPerRound[currentRound][letter].includes(e),
            );
            ...
            const equation =
                availableEqs[Math.floor(Math.random() * availableEqs.length)];
            ...
            ball.classList.forEach((cls) => {
                if (cls.startsWith("bingo-")) ball.classList.remove(cls);
            });
